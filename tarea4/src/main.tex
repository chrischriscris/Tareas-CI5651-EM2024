...
\begin{minted}{python}
    ...
    def ryanairDP(R: Conjunto[Maleta], memo: Arreglo[Entero]):
        ...
        min_t_1_maleta, min_t_2_maletas = Infinito, Infinito
        min_1_maleta_A, min_2_maletas_A, min_2_maletas_B = None, None, None
        for A in S-R:
            # Tiempo de O->A->O
            if 2 * g(O, A) < min_t_1_maleta:
                min_t_1_maleta = 2 * g(O, A)
                min_1_maleta_A = A

                for B in S-R-{A}:
                    # Tiempo de O->A->B->O
                    if  g(O, A) + g(A, B) + g(B, O) < min_t_2_maletas:
                        min_t_2_maletas =  g(O, A) + g(A, B) + g(B, O)
                        min_2_maletas_A, min_2_maletas_B = A, B

        caso1 = ryanairDP(R U {min1suitcasei}, memo) + min_t_1_maleta
        caso2 = ryanairDP(R U {min2suitcasesi, min2suitcasesj}, memo) + min_t_2_maletas

        memo[R] = min(caso1, caso2)
        return memo[R]
\end{minted}
