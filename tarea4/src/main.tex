...
% ----------------------------------------
% M = conjunto de maletas
% T[C] = Mejor tiempo acumulado al llevar las maletas en M - C
%
% T[M] = 0
% T[empty] = sol.
%
% T[C] = min(
%     T[C U {ei, ej}] + d(0, ei) + d(ei, ej) + d(ej, 0),
%     T[C U {ek} + 2d(0, el
% )

% parejaDP(S: Conjunto, f: funcion, p: Arreglo):
%     if S==E:
%         return p[s]
%
%     if p[S] != -1:
...
%     for el_i in M-S:
%         mejor = 0
%         for el_j in M-S-{el_i}:
%             if f(el_i, e_j) > mejor:
%                 mejor_el_j = el_j
%                 mejor = f(el_i, el_j)
%         p[s] = parejaDP(S U {el_i, el_j}, f, p) + mejor
%             return p[s]
%
...
%         min = base
%         for el_j in M-S-{el_i}:
%             dist = base + d(el_i, el_j) + d(el_j, 0)
%             if dist < min:
%                 mejor_el_j = el_j
%                 mejor = dist
%
%         if mejor == base:
%             p[C] = parejaDP(C U {el_i}, d, p) + base
%         else:
